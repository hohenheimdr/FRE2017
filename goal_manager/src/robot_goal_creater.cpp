...
public:
	...
	void read_robot_pose()
	{
		...
		try{
			...
			li.waitForTransform(fixed_frame,robot_frame,robot_pose.header.stamp,ros::Duration(0.1));	//3.0	
			...
			if(relative_goals)
				{
					try{
						...
						li.waitForTransform(fixed_frame,robot_frame+"_old",ros::Time(0),ros::Duration(0.1)); //3.0		
	...
	{	
		try{
				...
				if(pose_goal.header.frame_id!=fixed_frame)
				{
					...
					li.waitForTransform(fixed_frame,pose_goal.header.frame_id,ros::Time(0),ros::Duration(0.1));		//3.0
